...
#   2. Patched Jellyfin    — server + web patches applied to v10.11.11.
#
...
echo "==> 1/3  Building FFmpeg base image ($BASE_TAG) — this is the slow one (emulated)"
docker buildx build --platform linux/arm64 \
  -f "$REPO_DIR/Dockerfile.ffmpeg" \
  -t "$BASE_TAG" --load "$REPO_DIR"

echo "==> 2/3  Cloning Jellyfin $JELLYFIN_TAG and applying patches"
...
echo "==> 3/3  Building patched Jellyfin image ($TAG)"
docker buildx build --platform linux/arm64 \
